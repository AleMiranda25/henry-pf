  ...
  const postOrder = async () => {
    try {
      if (validateOrder(turnoSeleccionado)) {
        const res = await axios.post(`/order`, turnoSeleccionado);
  ...
  return (
    ...
    >
      ...
      >
        <button
          key="crearOrden"
          className="hover:text-[#5770F4] hover:opacity-80 bg-[rgb(32,33,35)] text-[whitesmoke] grid place-items-center rounded-btn w-24 h-4 sm:w-24 md:lg:w-24 sm:h-3 md:lg:h-6 font-[Oswald] text-[10px] sm:md:lg:text-[10px] font-bold opacity-95 z-auto"
          onClick={postOrder}
        >
          Confirma tu Cita
        </button>
